...
export const des = (key, message, encrypt, mode, iv, padding) => {
...
var des3iv = [1, 2, 3, 4, 5, 6, 7, 8 ];
des3iv = byteToString(des3iv);
export const DES3 = {
  //3DES加密，CBC/PKCS5Padding
  encrypt:function(key,input){
    // var genKey = genkey(key, 0, 24);
    // return btoa(des(genKey.key, input, 1, 1, des3iv, 1));
    // var genKey = generatorKey(key, 0, 24);
    return btoa(des("G8%*@l1b", input, 1, 1, des3iv, 1));
  },
  ////3DES解密，CBC/PKCS5Padding
  decrypt:function(key,input){
    // var genKey = genkey(key, 0, 24);
    // console.log(atob("amF2YXNjcmlwdA=="))
    var g = atob(input)
    // var g = BASE64.decoder(input);
    alert(g)
    var a = des(key, g, 0, 1, des3iv, 1);
    return a;
  }
};

export const  generatorKey = (len) => {
  len = len || 24;
